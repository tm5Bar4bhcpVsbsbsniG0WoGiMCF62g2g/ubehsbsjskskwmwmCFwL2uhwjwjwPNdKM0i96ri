# Code Taken From https://github.com/Lazy-Leecher/lazyleech/blob/master/lazyleech/plugins/pyexec.py
#
...
from userge import Message, memory_file, userge


@userge.on_cmd(
    "sh",
    about={
        "header": "run shell cmd",
        "description": "similar to shell cmd:-exec,eval,term",
        "usage": "{tr}sh [shell code]",
        "examples": "{tr}sh echo Hi" "  {tr}sh mv file1.txt file2.txt",
    },
)
async def run_code(client, message: Message):
    class UniqueExecReturnIdentifier:
        pass

    code = message.text[3:].strip()
    if not code:
